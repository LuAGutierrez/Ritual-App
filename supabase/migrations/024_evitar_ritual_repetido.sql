-- ============================================================
-- Migración 024: evitar que se repita el ritual del día anterior
--
-- Bug real detectado en producción: get_ritual_page_data (migración 017)
-- elige el ritual del día con `día_del_año % cantidad_de_rituales_no_premium`.
-- Esa cantidad no es estable -- cambia cada vez que se agrega contenido o se
-- marca/desmarca un ritual como premium -- así que un cambio de conteo justo
-- entre un día y el siguiente puede hacer que el módulo caiga en el mismo
-- offset dos días seguidos. Pasó de verdad: 15/08 y 16/08 mostraron
-- literalmente la misma pregunta a la misma pareja.
--
-- Fix: después de calcular el candidato con la fórmula de siempre, se
-- compara contra el ritual usado el día anterior (mismo criterio para toda
-- la app, ya que la selección no depende de couple_id). Si coincide, se
-- avanza una posición (con wraparound). No cambia el comportamiento en el
-- caso normal -- solo actúa como red de seguridad cuando la fórmula
-- colisiona.
-- ============================================================
